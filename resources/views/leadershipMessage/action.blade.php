    ...
    <ul class="dropdown-menu">
        ...
        <li>
            <a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#modal"
                onclick="loadModal('{{ route('leadership-messages.edit', $row->id) }}')">Edit</a>
        </li>
        @endcan
        @can('Delete Awards')
        <form action="{{ route('leadership-messages.destroy', $row->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?')">
            @csrf
            @method('DELETE')
