<script type="text/javascript">
    $(".modal-title").text("{{ __('Edit Product Category') }}");
</script>
...
<form action="{{ route('productCategories.update', $productCategory->id) }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="form-group mb-2">
        ...
        <input class="form-control" name="name" type="text" id="name" value="{{ $productCategory->name }}" />
    </div>
    ...
    <div class="form-group mb-2">
        ...
        <select name="status" id="status" class="form-control select2">
            ...
            <option value="{{ $key }}" {{ $productCategory->status?->value === $key ? 'selected' : '' }}>
                {{ $label }}
            </option>
            @endforeach
        </select>
