...
@section('content')
    <div class="container-fluid">
        ...
        <div class="align-items-center d-flex bg-white border-bottom pb-2 mb-3 main-content-card-header">
            <h4 class="main-card-title mb-0 flex-grow-1">{{ __('Product Brand') }}</h4>
            <div class="flex-shrink-0">
                <div class="app-search">
                    <div class="input-group">
                        @can('Add Product Brands')
                        <button class="btn btn-sm btn-success" type="button" id="button-addon2" data-bs-toggle="modal"
                            data-bs-target="#modal" onclick="loadModal('{{ route('productBrands.create') }}')"><i
...
@push('scripts')
    ...
    <script>
        $('#modal').on('hidden.bs.modal', function() {
            window.LaravelDataTables["productBrands-table"];
        });
    </script>
@endpush
